...
OUT=Path('reports/production-acceptance');checks=[];errors=[];details={};BASE='http://127.0.0.1:18095'
with sync_playwright() as p:
 ...
 page.route('**/production/full.glb',lambda r:r.fulfill(status=503,body='controlled GLB failure'))
 ...
 page.unroute('**/production/full.glb');page.locator('[data-action="retry-scene"]').click();page.wait_for_function('()=>wanderwiseDiagnostics().sceneState==="ready"');checks.append('GLB failure exposes bounded error and actual retry, no 2D fallback')
