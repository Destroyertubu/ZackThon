...
const bytes=fs.readFileSync(new URL('home-full.glb',base));
const jsonLength=bytes.readUInt32LE(12),gltf=JSON.parse(bytes.subarray(20,20+jsonLength).toString());
const nodes=[],config=JSON.parse(fs.readFileSync(new URL('scene.json',base)));
...
test('GLB 2 container, embedded images and finite accessor bounds',()=>{
 assert.equal(bytes.readUInt32LE(0),0x46546c67);assert.equal(bytes.readUInt32LE(4),2);assert.equal(bytes.readUInt32LE(8),bytes.length);
 for(const a of gltf.accessors)for(const v of [...(a.min||[]),...(a.max||[])])assert.ok(Number.isFinite(v));
 assert.ok(gltf.images.length>0);for(const image of gltf.images){assert.equal(image.mimeType,'image/png');assert.equal(typeof image.bufferView,'number');assert.equal(image.uri,undefined)}
 for(const v of gltf.bufferViews)assert.ok((v.byteOffset||0)+v.byteLength<=gltf.buffers[v.buffer].byteLength);
});
test('actual GLB retains whitelist facilities, safe spawn and Y-up metre scale',()=>{
...
test('all facility approaches share a traversable space; door is under ten seconds away',()=>{
 ...
 for(const [x,z] of [[-.76,-1.5],[-2.35,.95],[1.05,-1.55],[3.2,1.1],[-3.2,-2.5]])assert.ok(seen.has(key(Math.round(x/step),Math.round(z/step))),`unreachable ${x},${z}`);
 assert.ok(seen.get(key(-16,-12))/4.5<10);
});
test('room bounds replace old hardcoded boundary without clipping a legitimate route',()=>{
 const e={player:{x:0,y:0,z:4.45},kind:'home',scene};Engine.prototype.move.call(e,0,.2);assert.equal(e.player.z,4.45);Engine.prototype.move.call(e,0,-.3);assert.ok(e.player.z<4.45);
});
test('collision layers distinguish glass, frame, door leaf and camera ceiling',()=>{
 ...
 const ceiling=colliders.find(b=>b.id==='COL_ceiling');assert.equal(ceiling.player,false);assert.equal(ceiling.camera,true);
});
test('visible geometry and startup artifact have bounded size',()=>{
 let triangles=0;
