...
P=argparse.ArgumentParser();P.add_argument('--base-url',default='http://127.0.0.1:18093');P.add_argument('--soak-seconds',type=int,default=1200);P.add_argument('--headless',action='store_true');P.add_argument('--output-dir',type=Path);A=P.parse_args()
OUT=A.output_dir or ROOT/'docs/realistic-home/evidence';OUT.mkdir(parents=True,exist_ok=True)
...
with sync_playwright() as pw:
 ...
 try:
  ...
  facility('phone',{'x':3.2,'y':0,'z':2.2});assert '未开放' in page.locator('#overlay-root').inner_text();close();check('phone remains honestly unavailable')
  ...
  view('behind-phone',{'x':4.8,'y':0,'z':4.40},0,0,2);page.locator('[data-action="roam"]').click();page.wait_for_timeout(300);assert page.evaluate('wanderwiseDiagnostics().target')!='phone';page.keyboard.press('Escape');close();check('opaque wall blocks facility targeting')
