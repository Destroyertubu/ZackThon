...
let scripts=walk(path.join(root,'frontend')).filter(f=>/\.m?js$/.test(f));
for(let script of scripts){let r=spawnSync(process.execPath,['--check',script],{stdio:'inherit'});if(r.status)process.exit(r.status)}
console.log(`${scripts.length} ES modules passed syntax checks. Frontend is already the distributable; same-origin vendored libraries included; no build required by Python users.`);
