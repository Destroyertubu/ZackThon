...
export async function loadHome(signal){
 ...
 if(config.schemaVersion!==1||!/^\/static\/assets\/home\/home-(?:realistic-)?(corner|full)\.glb$/.test(config.model))throw Error('不支持的小屋配置');
 ...
 try{
  ...
  root.traverse(o=>{
   ...
   }else if(o.isMesh){const materials=Array.isArray(o.material)?o.material:[o.material];o.castShadow=!o.name.startsWith('VIS_exterior')&&!o.name.startsWith('VIS_drapery')&&!materials.every(m=>m.transparent);o.receiveShadow=!o.name.startsWith('VIS_drapery');
    // Keep GLTFLoader color/data semantics. Never reassign imported map colorSpace.
    for(const m of materials)if(m){if(o.name.includes('plants')||o.name.includes('window')||o.name.includes('drapery'))m.side=2;for(const value of Object.values(m))if(value?.isTexture)value.anisotropy=4;}
   }
  });
  ...
  return {physics,kind:'home',stage:config.stage,root,geometry:new Float32Array(0),colliders,targets,nodes:[],links:[],labels:targets.map(t=>({text:t.title,position:[t.position.x,t.position.y+.55,t.position.z],height:.30,target:t})),height:()=>0,bounds,boundary:8,spawn,camera:config.camera,dispose:()=>{physics?.dispose();disposeTree(root)},assetBytes:data.byteLength};
