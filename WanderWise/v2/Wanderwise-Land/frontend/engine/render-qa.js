...
export function installRenderQA(getEngine){
 ...
 window.wanderwiseRenderQA=Object.freeze({
  setView({position,yaw=0,pitch=.22,distance=3.8}){
   ...
   e.camera={yaw,pitch,distance};const p=e.player;e.eye=[p.x+Math.sin(yaw)*Math.cos(pitch)*distance,p.y+1.18+Math.sin(pitch)*distance,p.z+Math.cos(yaw)*Math.cos(pitch)*distance];
  },
