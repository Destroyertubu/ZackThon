...
export default function BackgroundMusic() {
  ...
  const globalMuted = usePersonalStore(state => state.data.settings.muted);
  const effectivelyMuted = preferences.muted || globalMuted;
  ...
  }, [preferences, effectivelyMuted]);

  useEffect(() => {
    mounted.current = true;
    wantsPlayback.current = preferencesRef.current.enabled;
    // Mirror the initial HTMLAudioElement.play() request in the transport controls.
  ...
  return (
    ...
    >
      ...
      >
        <source src="/galaxy/audio/day-one.mp3" type="audio/mpeg" />
        <source src="/galaxy/audio/day-one.flac" type="audio/flac" />
      </audio>
      <button
        type="button"
        ...
        <div className="music-projection" id={panelId} role="group" aria-label="背景音乐控制">
          ...
          <div className="music-volume-row">
            <button
              ...
              aria-label={effectivelyMuted ? "取消音乐静音" : "音乐静音"}
              aria-pressed={effectivelyMuted}
              onClick={() => { const muted = !effectivelyMuted; usePersonalStore.getState().settings({ muted }); setPreferences(value => ({ ...value, muted })); }}
            >
              {effectivelyMuted || preferences.volume === 0 ? <VolumeX size={16} /> : <Volume2 size={16} />}
            </button>
            <input
              type="range"
              min="0"
              ...
              onChange={(event) => { usePersonalStore.getState().settings({ muted: false }); setPreferences(value => ({ ...value, volume: Number(event.target.value) / 100, muted: false })); }}
            />
