...
type StellarTextProps = {
  ...
  className?: string;
  /** Optional presentation of the same text. Keep parsers in the calling scene. */
  content?: ReactNode;
};

/** DOM text remains selectable and accessible; sampled glyph dust supplies the transition. */
export default function StellarText({ text, phase, reducedMotion, className = '', content }: StellarTextProps) {
  const customContent = content != null;
  ...
  useEffect(() => {
    ...
    // The custom node represents `text`; its JSX identity may change every
    // camera update without changing glyphs. Only resample semantic changes.
  }, [text, phase, reducedMotion, fontRevision, customContent]);
  return <span className={`stellar-text stellar-text-${phase} ${className}`}>
    <span ref={contentRef} className="stellar-text-content">{content ?? text}</span>
