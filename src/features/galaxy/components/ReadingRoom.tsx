  ...
  return (
    ...
    >
      <section
        ...
        role="dialog"
        aria-modal="true"
        aria-label={answer.isExcerpt?'摘要与来源':'原文阅览'}
      ...
      >
        ...
        <header className="reader-topline">
          <span>
            <i />
            {answer.isExcerpt?'摘要与来源':'原文阅览'}
          </span>
          <div>
            <span className="reader-source-badge">
              {answer.sourceName??'知乎'} ·{" "}
              {answer.curated ? '精选阅读导引' : answer.workId
                ? "正文节选"
                : answer.isExcerpt
                  ? "搜索摘要"
                  : "原文"}
            ...
            <button
              ...
              aria-label={answer.isExcerpt && !answer.workId ? "关闭摘要阅读" : "关闭原文阅览"}
              onClick={onClose}
            >
              <X size={19} />
            </button>
        ...
        <div className="reader-scroll">
          <div className="reader-article-heading">
            ...
            <h1>{hasRichSyntax(answer.title) ? <RichText text={answer.title} inline /> : <LivingWords text={answer.title}/>}</h1>
          ...
          {answer.isExcerpt && (
            <p className="reader-content-note">
              {answer.curated ? '以下摘要与阅读导引由项目编写；完整作品请通过独立的原文链接阅读。' : answer.sourceName && answer.sourceName!=='知乎' ? '以下为来源摘要，完整内容请通过下方链接阅读。' : answer.workId
                ? "以下为知乎官方接口提供的正文节选，可能在段落中截断。"
                : "以下为知乎搜索返回的摘要。完整内容请通过原文链接阅读。"}
            </p>
          )}
          ...
          {answer.url && (
            <a
              ...
              rel="noreferrer"
            >
              {answer.sourceName ? '前往原始来源阅读' : answer.workId ? "查看知乎官方内容来源" : "前往知乎原文"}
