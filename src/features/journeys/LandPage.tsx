...
export default function LandPage(){
  ...
  return <main className="ms-world-page">
    ...
    <aside className="ms-land-compass"><Compass size={20}/><span>珍珠岸 · 暮色</span><div>{LAND_DEFINITION.stations.map(s=><button type="button" key={s.id} onClick={()=>interact(s.id)}>{s.title}</button>)}</div></aside>
