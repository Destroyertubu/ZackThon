...
export default function JourneyPage(){
  ...
  return <main className="ms-world-page">
    ...
    <aside className="ms-route-progress" aria-label="旅程进度"><span>{trip.progress.completed?<Check size={15}/>:<Compass size={15}/>} {trip.progress.visited.length} / 5 处停靠</span><div>{realm.stations.map(s=><button type="button" key={s.id} title={s.title} aria-label={`阅读${s.title}`} aria-pressed={trip.progress.visited.includes(s.id)} onClick={()=>interact(s.id)}>{trip.progress.visited.includes(s.id)?<Check size={16}/>:<Compass size={16}/>}</button>)}</div></aside>
