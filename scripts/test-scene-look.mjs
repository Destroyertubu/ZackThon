...
Object.defineProperty(globalThis, 'sessionStorage', { value: { getItem: () => 'rtx' }, configurable: true })
const cloudCanvas = new Element(), cloudTurns = []; document.activeElement = null
const cloudLook = bindSceneLook(cloudCanvas, { rotate: (x, y) => cloudTurns.push([x, y]), isPaused: () => false })
assert.equal(cloudCanvas.requests, 0, 'cloud route entry must wait for the viewing cursor to lock first')
emit(cloudCanvas, 'pointerenter', { pointerType: 'mouse', clientX: 100, clientY: 100 })
emit(cloudCanvas, 'pointermove', { pointerType: 'mouse', clientX: 900, clientY: 700, buttons: 0 })
assert.equal(cloudTurns.length, 0, 'remote absolute hover must never spin the camera')
key('KeyW', cloudCanvas); assert.equal(cloudCanvas.requests, 0, 'walking alone cannot lock only the remote cursor')
emit(cloudCanvas, 'pointermove', { pointerType: 'mouse', clientX: 200, clientY: 200, buttons: 1 })
emit(cloudCanvas, 'pointermove', { pointerType: 'mouse', clientX: 225, clientY: 208, buttons: 1 })
assert.deepEqual(cloudTurns.at(-1), [25, 8], 'forwarded touch drag stays available in the cloud')
key('KeyF', cloudCanvas); assert.equal(document.pointerLockElement, cloudCanvas)
emit(document, 'mousemove', { movementX: 8, movementY: -3 })
assert.deepEqual(cloudTurns.at(-1), [8, -3], 'viewer-confirmed cloud look uses relative motion')
key('KeyE', cloudCanvas)
assert.equal(document.pointerLockElement, null, 'E releases the cloud cursor even without a nearby interaction')
key('KeyF', cloudCanvas)
key('KeyH', cloudCanvas)
assert.equal(document.pointerLockElement, null, 'H releases the cloud cursor without needing an Escape action')
key('Escape', cloudCanvas)
emit(cloudCanvas, 'pointermove', { pointerType: 'mouse', clientX: 100, clientY: 100, buttons: 1 })
emit(cloudCanvas, 'pointermove', { pointerType: 'mouse', clientX: 120, clientY: 106, buttons: 1 })
assert.deepEqual(cloudTurns.at(-1), [20, 6], 'touch clients retain explicit drag after viewer connection sends Escape')
cloudLook.dispose(); delete globalThis.sessionStorage
console.log('Cloud scene look passed: route entry, hover, walking, explicit relative lock and touch drag.')
